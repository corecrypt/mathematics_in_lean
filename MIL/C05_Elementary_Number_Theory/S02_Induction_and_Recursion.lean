...
theorem pow_two_le_fac (n : ℕ) : 2 ^ (n - 1) ≤ fac n := by
  ...
  apply Nat.mul_le_mul (Nat.le_add_left 2 n) ih
...
theorem succ_mul (m n : MyNat) : mul (succ m) n = add (mul m n) n := by
  induction' n with n ih
  . rw [mul, add, mul]
  rw [mul, ih, mul, add_comm, succ_add, ← add_comm n.succ, succ_add]
  rw [← add_comm n, ← add_comm m,]
  simp
  rw [add_comm n, add_comm n, add_assoc]
